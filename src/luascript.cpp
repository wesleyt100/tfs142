...
PersonalStore LuaScriptInterface::getPersonalStore(lua_State *L, int32_t arg)
{
	PersonalStore ps;
	ps.name = getFieldString(L, arg, "name");
	ps.mode = getField<uint8_t>(L, arg, "mode");
...
void LuaScriptInterface::pushPersonalStore(lua_State* L, const PersonalStore& ps)
{
	lua_createtable(L, 0, 2);
	setField(L, "name", ps.name);
	setField(L, "mode", ps.mode);
}
...
void LuaScriptInterface::registerFunctions()
{
	...
	registerMethod("Creature", "getPersonalStore", LuaScriptInterface::luaCreatureGetPersonalStore);
	registerMethod("Creature", "setPersonalStore", LuaScriptInterface::luaCreatureSetPersonalStore);
	...
	registerMethod("Player", "getItemByAttribute", LuaScriptInterface::luaPlayerGetItemByAttribute);
	registerMethod("Player", "getItemFromPosition", LuaScriptInterface::luaPlayerGetItemFromPosition);
...
int LuaScriptInterface::luaItemGetAttributes(lua_State *L)
{
	// item:getAttributes()
	Item *item = getUserdata<Item>(L, 1);
	if (item)
	{
		...
		pushString(L, db.escapeBlob(attributes, attributesSize));
		return 1;
	}
	else
	{
...
int LuaScriptInterface::luaItemLoadAttributes(lua_State *L)
{
	// item:loadAttributes(resultId)
	Item *item = getUserdata<Item>(L, 1);
	if (item)
	{
		DBResult_ptr result = ScriptEnvironment::getResultByID(getNumber<uint32_t>(L, 2));
		if (!result)
		{
			lua_pushnil(L);
			return 1;
		...
		{
			std::cout << "WARNING: Serialize error in loadAttributes" << std::endl;
			lua_pushnil(L);
			return 1;
		}
	}
...
int LuaScriptInterface::luaCreatureGetPersonalStore(lua_State *L)
{
	// creature:getPersonalStore()
	const Creature *creature = getUserdata<const Creature>(L, 1);
	if (creature)
	{
		pushPersonalStore(L, creature->getPersonalStore());
	}
	else
	{
		lua_pushnil(L);
 ...
 int LuaScriptInterface::luaCreatureSetPersonalStore(lua_State *L)
 {
 	// creature:setPersonalStore(personal_store)
 	Creature *creature = getUserdata<Creature>(L, 1);
 	if (creature)
 	{
 		creature->setPersonalStore(getPersonalStore(L, 2));
 		g_game.internalCreatureChangePersonalStore(creature, creature->personalStore);
		pushBoolean(L, true);
	}
 	else
	{
...
{
	// player:toggleWing(bool, uint8_t)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	bool enable = getBoolean(L, 2, true);
	uint8_t wingId = getNumber<uint8_t>(L, 3, 1); // padrão: 1

	player->setCurrentWing(enable ? wingId : 0);
	lua_pushboolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaPlayerGetItemByAttribute(lua_State *L)
{
	// player:getItemByAttribute(key, value)
	Player *player = getUserdata<Player>(L, 1);
	if (!player)
	{
		lua_pushnil(L);
	...
	if (isNumber(L, 2))
	{
		item = g_game.findItemOfAttribute(player, getNumber<int64_t>(L, 2), value);
	}
	else if (isString(L, 2))
	{
		item = g_game.findItemOfAttribute(player, getString(L, 2), value);
	}
	else
	{
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerGetItemFromPosition(lua_State *L)
{
	// player:getItemFromPosition(pos, stackpos)
	Player *player = getUserdata<Player>(L, 1);
	if (!player)
	{
		lua_pushnil(L);
	...
	const Position &position = getPosition(L, 2);
	uint8_t stackpos = getNumber<uint8_t>(L, 3);
	...
	Thing *thing = g_game.internalGetThing(player, position, fromIndex, 0, STACKPOS_MOVE);
	if (!thing)
	{
		lua_pushnil(L);
		return 1;
