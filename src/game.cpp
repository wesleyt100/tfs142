                           ...
                           uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count)
{
	...
	if (Creature* movingCreature = thing->getCreature()) {
		...
		// 🔒 Bloquear qualquer tentativa de mover item para o chão (inclui tiles e containers no chão)
		if (toCylinder->getParent() == nullptr) {
			player->sendTextMessage(MESSAGE_STATUS_SMALL, "You cannot place items on the ground.");
			return;
		}
...
ReturnValue Game::internalMoveCreature(Creature* creature, Direction direction, uint32_t flags /*= 0*/)
{
	...
	{
		if (player->getPersonalStore().mode == 1)
		{
                     ...
                     const std::string& receiver, const std::string& text)
{
	...
	switch (type) {
		case TALKTYPE_SAY:
			g_game.addAnimatedText(text, player->getPosition(), TEXTCOLOR_WHITE_EXP);
			break;
...
void Game::internalCreatureChangePersonalStore(Creature *creature, const PersonalStore &ps)
 {
	if (creature->isInvisible())
 	{
 		return;
 	...
 	for (Creature *spectator : spectators)
 	{
 		spectator->getPlayer()->sendCreatureChangePersonalStore(creature, ps);
 	}
 }

Item *Game::findItemOfAttribute(Cylinder *cylinder, int64_t key, const std::string value) const
{
	if (cylinder == nullptr)
	{
		return nullptr;
	...
	for (size_t i = cylinder->getFirstIndex(), j = cylinder->getLastIndex(); i < j; ++i)
	{
		...
		if (attr)
		{
			if (attr->value.type() == typeid(std::string) && boost::get<std::string>(attr->value) == value)
			{
				return item;
			}
		}
...
Item *Game::findItemOfAttribute(Cylinder *cylinder, const std::string key, const std::string value) const
{
	if (cylinder == nullptr)
	{
		return nullptr;
