...
void Combat::doTargetCombat(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
    ...
    // BLOQUEIO E MODIFICAÇÃO DE DANO
    bool success = false;
    if (damage.primary.type != COMBAT_MANADRAIN) {
        ...
        // Redução de dano PVP
        if (casterPlayer && targetPlayer && casterPlayer != targetPlayer && targetPlayer->getSkull() != SKULL_BLACK && damage.primary.type != COMBAT_HEALING) {
            damage.primary.value /= 2;
            damage.secondary.value /= 2;
        }

        // Critical hit
        if (casterPlayer && !damage.critical && damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
            ...
            if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
                damage.primary.value += std::round(damage.primary.value * (skill / 100.0));
                damage.secondary.value += std::round(damage.secondary.value * (skill / 100.0));
                damage.critical = true;
            }
        }

        // APLICAÇÃO DE ABSORÇÃO DO TARGET
        if (targetPlayer) {
            auto applyAbsorb = [](int32_t& value, int16_t absorbPercent, int16_t fieldAbsorbPercent) {
                if (absorbPercent != 0) value -= std::round(value * (absorbPercent / 100.0));
                if (fieldAbsorbPercent != 0) value -= std::round(value * (fieldAbsorbPercent / 100.0));
            };

            applyAbsorb(damage.primary.value,
                        targetPlayer->abilities.absorbPercent[combatTypeToIndex(damage.primary.type)],
                        targetPlayer->abilities.fieldAbsorbPercent[combatTypeToIndex(damage.primary.type)]);

            applyAbsorb(damage.secondary.value,
                        targetPlayer->abilities.absorbPercent[combatTypeToIndex(damage.secondary.type)],
                        targetPlayer->abilities.fieldAbsorbPercent[combatTypeToIndex(damage.secondary.type)]);
        }
    ...
    // APLICAÇÃO DE CONDITIONS, CRITICAL, LEECH ETC
    if (success) {
        ...
        if (!damage.leeched && damage.primary.type != COMBAT_HEALING && casterPlayer && damage.origin != ORIGIN_CONDITION) {
            ...
            if (casterPlayer->getHealth() < casterPlayer->getMaxHealth()) {
                ...
                if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
                    leechCombat.primary.value = std::round(totalDamage * (skill / 100.0));
