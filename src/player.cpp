...
void Player::applyItemAttributes(const ItemType* item)
{
    for (int i = 0; i < COMBAT_COUNT; i++) {
        abilities.absorbPercent[i] += item->absorbPercent[i];
        abilities.fieldAbsorbPercent[i] += item->fieldAbsorbPercent[i];
    }

}

void Player::removeItemAttributes(const ItemType* item)
{
    for (int i = 0; i < COMBAT_COUNT; i++) {
        abilities.absorbPercent[i] -= item->absorbPercent[i];
        abilities.fieldAbsorbPercent[i] -= item->fieldAbsorbPercent[i];
...
{
	if ((OTSYS_TIME() - lastToggleMount) < 1000 && !wasMounted) {
